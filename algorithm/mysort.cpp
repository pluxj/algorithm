...
namespace myalgoritm
{
    ...
    int CSort::binarySearch(int *arr,int n,int target)
    {
        //在 arr[l...r] 区间内找到 target，arr 是有序的
        int l = 0; int r = n-1;
        while(l <= r)
        {
            int mid = l + (r-l) /2;
    ...
    int CSort::binarySearchD(int *arr,int n,int target)
    {
        return _binaryD(arr,0,n-1,target);
    }

    int CSort::_binaryD(int *arr,int l,int r,int target)
    {
        if(l > r)
        {
            return -1;
        ...
        if(arr[mid] > target)
        {
            _binaryD(arr,l,mid-1,target);
        }
        else
        {
            _binaryD(arr,mid+1,r,target);
        }
    }
